...
const mapGames = (webpage: string, siteUrl: string) => {
  ...
  const games = gamesSelector
    .map((_i, el) => {
      ...
      const image = imageData.substring(0, imageData.indexOf(" ")).replace(
        "150x",
        "350x",
      ) || "";
      const url =
        siteUrl + $(el).find("a.grid-view-item__link").first().attr("href") ||
        "";
...
const getStrictlyLimitedGamesResponses = async (siteUrl: string) => {
  const firstPageResponse = await fetch(
    `${siteUrl}/collections/nintendo-switch`,
  );
  const secondPageResponse = await fetch(
    `${siteUrl}/collections/nintendo-switch?page=2`,
  );

  return { firstPageResponse, secondPageResponse };
};

const getStrictlyLimitedGamesTexts = async (
  firstPage: Response,
  secondPage: Response,
) => {
...
export const getGames = async (siteUrl: string) => {
  try {
    const strictlyLimitedGamesResponses =
      await getStrictlyLimitedGamesResponses(siteUrl);
    ...
    return [
      ...mapGames(strictlyLimitedGamesTexts.firstPageText, siteUrl),
      ...mapGames(strictlyLimitedGamesTexts.secondPageText, siteUrl),
    ];
  } catch (error) {
