...
const getGames = async () => {
  const superRareGamesResponse = await fetch('https://superraregames.com/collections/all')
  if (superRareGamesResponse.status === 404) {
    return []
  }
  const superRareGamesText = await superRareGamesResponse.text()
  const superRareGames = mapGames(superRareGamesText, 'https://superraregames.com')
